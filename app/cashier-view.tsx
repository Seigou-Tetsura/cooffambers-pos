              ...
              <section key={category.id} className="bg-white rounded-xl border border-stone-200 shadow-[0_1px_3px_rgba(40,33,26,0.05)] p-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                  {itemsInCategory.map((item) => {
                    ...
                    if (hasTemp) {
                      return (
                        <div key={item.id} className={`flex flex-col border rounded-lg overflow-hidden transition-all duration-200 ${item.soldOut ? "border-stone-200" : isSelectedAny ? "border-[#8a5a3b] shadow-[0_0_0_1px_rgba(138,90,59,0.4)] z-10" : "border-stone-200"}`}>
                          ...
                          <div className="flex divide-x divide-stone-200 border-t border-stone-100">
                            {hasHot && (
                              <button onClick={() => addToCart(item, "Hot")} disabled={item.soldOut} className={`flex-1 flex items-center justify-between px-3 py-2.5 transition-all disabled:cursor-not-allowed disabled:opacity-50 ${hotQty > 0 ? "bg-[#8a5a3b]/[0.08]" : "bg-white hover:bg-red-50/60 active:scale-[0.99]"}`}>
                            ...
                            {hasIce && (
                              <button onClick={() => addToCart(item, "Ice")} disabled={item.soldOut} className={`flex-1 flex items-center justify-between px-3 py-2.5 transition-all disabled:cursor-not-allowed disabled:opacity-50 ${iceQty > 0 ? "bg-[#8a5a3b]/[0.08]" : "bg-white hover:bg-blue-50/60 active:scale-[0.99]"}`}>
                                <div className="flex items-center gap-1.5">
                    ...
                    return (
                      <button key={item.id} onClick={() => addToCart(item)} disabled={item.soldOut} className={`relative flex justify-between items-center px-4 py-3.5 border rounded-lg text-left transition-all duration-200 ${item.soldOut ? "bg-stone-50 border-stone-200 cursor-not-allowed" : normalQty > 0 ? "bg-[#8a5a3b]/[0.04] border-[#8a5a3b] shadow-[0_0_0_1px_rgba(138,90,59,0.4)] z-10" : "bg-white border-stone-200 hover:border-[#8a5a3b]/50 hover:bg-[#8a5a3b]/[0.02] active:scale-[0.99]"}`}>
